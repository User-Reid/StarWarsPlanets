...
public class App
{
  ...
  public async Task Run(IClient _apiInterface)
  {
    ...
    var userInputPropertyChoice = Console.ReadLine();

    switch (userInputPropertyChoice)
    {
      case "Population":
        {
          FindHighestPopulationPlanet(planetsDirectory);
          break;
        }
    }
  }

  public void FindHighestPopulationPlanet(Dictionary<string, Root> planetsDirectory)
  {
    var maxPopulationPlanet = planetsDirectory.OrderByDescending(planet => int.Parse(planet.Value.population)).First().Key;
    System.Console.WriteLine($"{maxPopulationPlanet}");
  }
}
